...
// Note: the cache directory is created when the server starts.
...
const doResize = async (
  ...
  try {
    ...
    const targetPath = path.join(__dirname,"cache",`${data.filename}-${width}X${height}.${data.extension}`);
    ...
    return path.join("cache",`${data.filename}-${width}X${height}.${data.extension}`);
  } catch (err) {
    console.log(err);
  }
};
